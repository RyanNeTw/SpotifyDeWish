...
async function getQuery(searchWord){
  console.log(searchWord.target.value)
  response.value = await search(searchWord.target.value, 'album')
}
  ...
  <main class="px-16 pt-8 pb-32 bg-stone-900">  
    ...
    <ul v-if="response != 0" class="flex flex-wrap gap-12">
      <li v-for="album in response.albums.items" :key="album">
        <router-link :to="`/albums/?query=${album.id}`" class="bg-stone-800 hover:bg-stone-700 transition-colors flex flex-col justify-center rounded-lg">
